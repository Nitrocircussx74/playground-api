...
- **Testing Coverage**: **Full API Integration & Unit Testing ด้วย Jest** (373/373 Test Cases Passed — รันด้วย `yarn jest --runInBand` เพื่อกัน Test คนละไฟล์แย่ง DB State กันเวลารันขนาน; **หมายเหตุ**: ถ้ารันแบบไม่ใส่ `--forceExit` แล้ว Jest ค้างไม่ยอม Exit หลังเทสต์ผ่านหมดแล้ว (ขึ้น Warning "Have you considered using `--detectOpenHandles`") เป็นปัญหา Pre-existing ของ Open Handle บางจุด (สงสัย Prisma Connection Pool หรือ `express-rate-limit` Timer) ไม่ใช่ Test พังจริง ใส่ `--forceExit` ต่อท้ายเพื่อดูผลสรุปได้ปกติ; ดูรายละเอียดล่าสุดใน `docs/ACTIVITY_LOG.md` Phase 16; ตัวเลข 19/19 เดิมคือช่วงเริ่มโปรเจกต์ก่อนขยายเป็นระบบหอพักเต็มรูปแบบ)
- **Security & Billing Audit (Phase 23, 2026-09-25)**: ตรวจ flow ทั้งระบบแล้วแก้เป็น 4 เฟส (branch `fix/security-phase-0-1` → `fix/billing-phase-2` → `fix/auth-phase-3`, PR รวม `playground-api#5` + `playground-frontend#10`) — เทสต์ backend ใช้ DB แยก `playground_test` เท่านั้น (`tests/setup.js` ปฏิเสธถ้าไม่ตั้ง `TEST_DATABASE_URL` ห้ามชี้ไป `playground_db`), Access Token ถูกยืนยัน claims กับ DB ทุก Request (`authService.resolveCurrentClaims`), Refresh Token มี `familyId`/`usedAt` (Rotation + Reuse Detection: ใช้ Token ที่หมุนแล้วซ้ำหลัง Grace Window 10 วินาทีเพิกถอนทั้งตระกูล), Role `admin`/`manager` เข้าถึงได้เฉพาะตึกใน `UserBuildingPermission` ส่วน `owner`/`super_admin` ทุกตึก (`room_owner`/`investor` ผ่าน `entityParam` โดยไม่ถูกตรวจตึก ต้องเช็ค `ownerId` ใน Controller เอง), อัตราน้ำไฟ/วันครบกำหนดใช้ `BuildingSetting` เป็นต้นทางเดียว (`billingService.getBillingRates`), `dueDate` = วันที่ `dueDateDay` ของเดือนถัดจากรอบบิล (`dueDateForCycle`), ผู้เช่าใหม่เริ่มนับมิเตอร์จาก `LeaseContract.initialWaterReading/initialElectricReading` (`getPreviousReading`), จบสัญญาทุกทางต้องผ่าน `tenancyService.releaseRoomTenancy` (ปิดผู้อยู่ร่วม + การผูก LINE), URL ไฟล์อัปโหลดสร้างจาก `PUBLIC_BASE_URL` (`utils/publicUrl.js`) ไม่เชื่อ Host header, นามสกุลไฟล์อัปโหลดมาจาก MIME ที่ตรวจแล้วเท่านั้น, ตัดการ Login ด้วย Google ออกแล้ว
- **Condo/Village Modules (Phase 16)**: เพิ่ม 6 Models ใหม่รองรับ Property Type นอกเหนือหอพัก — `Facility`/`FacilityBooking` (จองพื้นที่ส่วนกลาง, เช็คช่วงเวลาซ้อนทับด้วย Interval Overlap Query), `Vehicle`/`Visitor` (ทะเบียนรถต้องอนุมัติ PENDING/APPROVED/REJECTED แยกจากแขกมาเยือนแบบไม่ต้องอนุมัติ EXPECTED/CANCELLED), `Poll`/`PollVote` (โหวต 1 คน 1 เสียง, unique `[pollId, tenantId]`) — ทุกโมดูลตามรูปแบบเบาแบบ `parcelController.js` (ไม่มี Service Layer/Zod Validator) โดยตั้งใจ ให้สอดคล้องกับของจริงในระบบ ไม่ใช่ตามกฎ Layered Architecture ใน `CLAUDE.md`
- **`requireFeatureMiddleware.js` (ใหม่)**: จุดแรกที่ `FeatureToggle` มีผลบังคับจริงฝั่ง Backend (เดิมทุกฟีเจอร์ก่อนหน้านี้ เช่น `ENABLE_PARCEL_NOTIFY`, เป็นแค่ Frontend `v-if` ซ่อน UI เฉยๆ ยิง API ตรงได้อยู่ดีแม้ปิด Toggle ไว้) ใช้กับ 3 Route เขียนของ Phase 16 เท่านั้น (`ENABLE_FACILITY_BOOKING`, `ENABLE_VEHICLE_MANAGEMENT`, `ENABLE_VOTING`) — ใช้เฉพาะ Route ที่เป็น Write ไม่ใช้กับ GET/List
...
- **Default `themeColor`**: `#0E7490` (Horspace Brand Teal, Sample จากโลโก้จริง) ทุกจุด Fallback ทั้ง Backend/Frontend — ไม่ใช่ `#3B82F6` (Blue) เดิมอีกต่อไป (ยกเว้น Preset ตัวเลือกสีให้แอดมินเลือกเองใน `BuildingSettingsView.vue` ที่ยังมี "น้ำเงิน (Blue)" เป็นตัวเลือกหนึ่งตามปกติ)
- **In-App Notification Bell (Phase 17)**: `NotificationLog` เดิม (เก็บประวัติ LINE Push Admin→Tenant) ถูก Reuse เป็น Source เดียวให้กระดิ่ง Admin CMS + Tenant LIFF ด้วย — เพิ่มแค่คอลัมน์ `readAt` ไม่สร้างตารางใหม่ แยก Audience ด้วยลิสต์ `ADMIN_ALERT_TYPES` ใน `lineService.js` (`MAINTENANCE_NEW`/`SLIP_UPLOADED`/`FACILITY_BOOKING_NEW`/`VEHICLE_NEW`/`TENANT_ONBOARDED` = เหตุการณ์ที่ Tenant ทำแล้วต้องให้ Admin มาดู, Type อื่นทั้งหมด = ข้อความที่ระบบส่งออกไปหา Tenant) — `readAt` เป็น Shared State ต่อตึก/ต่อผู้เช่า ไม่ใช่ต่อแอดมินแต่ละคน (แอดมินหลายคนเห็นสถานะอ่านร่วมกัน ตั้งใจไม่ทำ Per-Admin Read State เพราะเกินจำเป็นสำหรับสเกลหอพัก) (**Gap ปิดแล้วใน Phase 19**: `sendAnnouncementBroadcast`/`createAnnouncement` บันทึก `NotificationLog` แล้ว ประกาศข่าวสารขึ้นกระดิ่ง Tenant ปกติ)
- **LIFF Building Owner Dashboard (Phase 18)**: Mobile Portal สำหรับเจ้าของตึก/ผู้บริหารบน LINE LIFF — 3 Phase ครบในเซสชันเดียว: (1) Executive Snapshot (Metric Cards: รายรับ/รับชำระ/ค้าง, Occupancy Rate, Counter Badges), (2) Quick Approvals (Slip Verification Queue พร้อม Approve/Reject+LINE push, Urgent Maintenance List, Expiring Leases Alert), (3) Multi-Building Switcher + Dual-Role Mode Switcher (Owner↔Tenant pill สลับโดยไม่ reload) — `dashboardController.js` ตรวจ `UserBuildingPermission` ก่อนคืนข้อมูล, `tenantAuthService.js` คืน `availableRoles`+`tenantId` เพื่อ resolve Dual-Role — ไม่มี Endpoint ใหม่ Reuse ทั้งหมด
- **Broadcast Building-Scope Enforcement (Phase 20)**: `announcementController._resolveAllowedBuildingId()` บังคับให้แอดมิน/แมเนเจอร์ที่ไม่ใช่ Super Admin ส่งประกาศได้แค่ตึกที่มีสิทธิ์ใน `UserBuildingPermission` เท่านั้น (เดิมไม่เช็คเลย + Fallback เดาตึกแรกในระบบตอนไม่ระบุ `buildingId` มา เป็นช่องโหว่ IDOR) ใช้ Pattern เดียวกับ `dashboardController.getSummary` — ดูรายละเอียดเต็มใน `docs/ACTIVITY_LOG.md` Phase 20
...
# การรันชุดทดสอบทั้งหมด (373/373 Passed ล่าสุด)
yarn test
